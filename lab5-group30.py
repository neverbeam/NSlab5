...
## NAME: Martijn Dortmond and Danyllo Verweij
## STUDENT ID: 10740406, 10770488
import sys
import struct
import select
from socket import *
...
def echoSend(peer, initiator, sequence, operation, payload=0):
    ...
    for addres in neighbors:
        ...
        if addres !=  father:
            message = message_encode(MSG_ECHO, sequence, initiator, neighbors[addres], operation, args.range, payload)
...
def echoReceive(peer, addres, initiator, sequence, operation):
    ...
    if (initiator,sequence) not in lookuptable:
        ...
        if len(neighbors) == 1:
            if operation == OP_SIZE:
                ...
                lookuptable[(initiator,sequence)][2] = value
            message = message_encode(MSG_ECHO_REPLY, sequence, initiator, pos, operation, args.range, lookuptable[(initiator,sequence)][2])
    ...
    # Directly reply if I allready have got this echo message.
        ...
        if operation == OP_SIZE:
            ...
            payload = 0
        message = message_encode(MSG_ECHO_REPLY, sequence, initiator, pos, operation, args.range, payload)
        peer.sendto(message, addres)
...
def comparerange(xi, yi, addres, peer):
    ...
    if ((pos[0] - xi) ** 2 + (pos[1] - yi)**2) ** 0.5 < args.range:
        message = message_encode(1,0,(xi,yi),pos, 0, args.range)
        print "send pong"
        peer.sendto(message,addres)
...
def neighbordiscovery(peer,restart):
    global neighbors
    message = message_encode(MSG_PING, 0, pos, pos, 0, args.range)
