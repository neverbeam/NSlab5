...
public class WordCountTool extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		if (conf.get("mapreduce.framework.name").equals("yarn")) {
			...
			job.addCacheFile(new URI("/projects/cpp2015/englishPCFG.ser.gz"));
		} else if (conf.get("mapreduce.framework.name").equals("local")) {
			job.addCacheFile(new URI("sentiment.ser.gz"));
			job.addCacheFile(new URI("englishPCFG.ser.gz"));
		}
